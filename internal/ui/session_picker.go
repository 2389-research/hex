...
func (i sessionItem) Description() string {
	...
	// Format: "Updated: 2h ago • Model: claude-sonnet-4 • ID: conv-123..."
...
func NewSessionPicker(conversations []*storage.Conversation) SessionPicker {
	// Convert conversations to list items, with "New Session" at the top
	items := make([]list.Item, len(conversations)+1)
	items[0] = sessionItem{isNewItem: true}
	for i, conv := range conversations {
		items[i+1] = sessionItem{conv: conv}
	}
	...
	l.AdditionalShortHelpKeys = func() []key.Binding {
		return []key.Binding{
			...
			key.NewBinding(
				key.WithKeys("esc", "q"),
				key.WithHelp("esc/q", "new session"),
			),
		}
	}
...
// SelectedID returns the selected conversation ID (empty if new session)
func (m SessionPicker) SelectedID() string {
	return m.selected
}

// IsNewSession returns true if user wants a new session
func (m SessionPicker) IsNewSession() bool {
	return m.newSession
}
