...
`
	...
	assert.Equal(t, "test-key-123", cfg.ProviderConfigs["anthropic"].APIKey)
...
func TestConfigFromEnv(t *testing.T) {
	...
	_ = os.Setenv("ANTHROPIC_API_KEY", "env-key-456")
	...
	cfg, err := core.LoadConfig()
	require.NoError(t, err)
	assert.Equal(t, "env-key-456", cfg.ProviderConfigs["anthropic"].APIKey)
...
func TestConfigPrecedence(t *testing.T) {
	...
	configTOML := `[providers.anthropic]
api_key = "file-key"
`
	err := os.WriteFile(configPath, []byte(configTOML), 0600)
	require.NoError(t, err)
	...
	_ = os.Setenv("HEX_CONFIG_PATH", configPath)
	_ = os.Setenv("ANTHROPIC_API_KEY", "env-key")
	defer func() { _ = os.Unsetenv("HEX_CONFIG_PATH") }()
	defer func() { _ = os.Unsetenv("ANTHROPIC_API_KEY") }()

	cfg, err := core.LoadConfig()
	require.NoError(t, err)
	assert.Equal(t, "env-key", cfg.ProviderConfigs["anthropic"].APIKey, "env var should override file")
}
...
func TestConfigFromDotEnv(t *testing.T) {
	...
	dotEnvContent := `ANTHROPIC_API_KEY=dotenv-key-789
...
`
	...
	cfg, err := core.LoadConfig()
	require.NoError(t, err)
	assert.Equal(t, "dotenv-key-789", cfg.ProviderConfigs["anthropic"].APIKey)
	assert.Equal(t, "claude-sonnet-4-5-20250929", cfg.Model)
}
