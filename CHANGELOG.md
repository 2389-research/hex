...
Jeff v1.0.0 is the first production-ready release, consolidating all features from v0.1.0 through v0.6.0 into a stable, enterprise-grade AI CLI tool.
...
None critical. See [GitHub Issues](https://github.com/harper/jefft/issues) for minor enhancements.
...
Three parallel development efforts transforming Jeff from a feature-complete prototype into a production-ready, enterprise-grade AI CLI tool:
...
- 6 installation methods:
  ...
  - Homebrew: `brew install harper/tap/jefft`
  - Go install: `go install github.com/harper/jefft/cmd/jefft@latest`
  - Pre-built binaries from GitHub Releases
  - Build from source: `make install`
  - Docker: `docker pull ghcr.io/harper/jefft:latest`
...
Four major enhancements expanding Jeff's capabilities for background execution, streaming, persistence, and external tool integration:
...
- Tool adapter bridging MCP tools to Jeff Tool interface
- CLI commands:
  - `jefft mcp add <name> <command> [args...]` - Register MCP server
  - `jefft mcp list` - Show configured servers
  - `jefft mcp remove <name>` - Unregister server
...
**Integration**
- Modified `cmd/jefft/root.go` to load MCP tools after built-in tools
...
- internal/mcp/tool_adapter.go - MCP→Jeff bridge
- internal/mcp/loader.go - Automatic tool loading
- internal/mcp/mock_server_test.go - Test infrastructure
- cmd/jefft/mcp.go - CLI commands
- cmd/jefft/mcp_integration_test.go - Integration tests
...
Seven new tools expanding Jeff's capabilities for interactive decision-making, research, and advanced execution:
...
- User-Agent header set to "Jeff/1.0"
...
Three new tools complete Jeff's core tool suite:
...
#### Storage System
- SQLite-based conversation persistence at `~/.jeff/jefft.db`
...
- Setup command (`jefft setup-token`) for API key configuration
- Doctor command (`jefft doctor`) for health checks
...
[0.2.0]: https://github.com/harper/jefft/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/harper/jefft/releases/tag/v0.1.0
