...
import (
	...
	"github.com/harper/jeff/internal/mcp"
...
Examples:
  jeff mcp add weather node weather-server.js
  jeff mcp add database python -m database_server --port 8080
  jeff mcp add files /usr/local/bin/file-server`,
...
func runMCPList(cmd *cobra.Command, baseDir string) error {
	...
	if len(servers) == 0 {
		...
		cmd.Println()
		cmd.Println("Add a server with: jeff mcp add <name> <command> [args...]")
		return nil
	}
